...
Usage:
    python update_frontend.py [--db /path/to/paper_trades_v2.db] [--out results/live_paper_trades.json]

Typical cron (every 30 min on gpu-server2):
    */30 * * * * cd /workspace/fx_ml && python update_frontend.py >> /tmp/update_frontend.log 2>&1

After running, push results/live_paper_trades.json to the UTradeDashboard repo:
    cp results/live_paper_trades.json /path/to/UTradeDashboard/results/
    cd /path/to/UTradeDashboard && git add -A && git commit -m "chore: update live paper trades" && git push
"""
...
def query_all_signals(db_path):
    """Query paper_trades_v2.db grouped by portfolio_signal."""
    conn = sqlite3.connect(db_path)
    conn.row_factory = sqlite3.Row
    cur = conn.cursor()

    # Get all distinct portfolio_signals that have at least one closed trade
    cur.execute("""
...
def main():
    ...
    signals_out = query_all_signals(args.db)
